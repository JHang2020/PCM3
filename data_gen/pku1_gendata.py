...
def read_data(data_path, name, max_body=4, num_joint=25):  # top 2 body
    ...
    seq_data = np.loadtxt(os.path.join(data_path, filename))
    label = np.loadtxt(os.path.join('/mnt/netdisk/Datasets/088-PKUMMD/PKUMMDv1/Train_Label_PKU_final/',filename), delimiter=',')
...
def gendata(data_path, out_path, benchmark='xview', part='eval'):
    ...
    with open('/mnt/netdisk/zhangjh/Code/CMD-main/data_gen/pku_cross_view_v2.txt', 'r') as f:
        lines = f.readlines()
        training_views = lines[1].strip('\n').split(', ')
    with open('/mnt/netdisk/zhangjh/Code/skeleton_detection/detection/tools/pkuv1_cross_subject.txt', 'r') as f:
        lines = f.readlines()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='/mnt/netdisk/Datasets/088-PKUMMD/PKUMMDv1/PKU_Skeleton_Renew')
    parser.add_argument('--out_folder', default='/mnt/netdisk/zhangjh/data/CMD/PKUMMD1/')
    benchmark = ['xsub', ]
